<?php
	...
	if($tipo_comprobante == '1')
	{
		...
		$pdf->Text(3.8, $get_YH  + 5, 'CAI : '.$serie);
		$pdf->Text(3.8, $get_YH  + 10, 'Fecha limite emision : '.$serie);
		$pdf->Text(3.8, $get_YH  + 15, 'Rango autorizado : '.$serie);
		$pdf->Text(3.8, $get_YH  + 20, 'Orden compra exenta : ');
		$pdf->Text(3.8, $get_YH  + 25, 'Constancia Reg. exonerado : ');
		$pdf->Text(3.8, $get_YH  + 30, 'Registro SAG : ');

		$pdf->Text(2, $get_YH + 32 , '
			------------------------------------------------------------------');
		$pdf->SetFont('Arial', 'B', 8.5);
		$pdf->Text(3.8, $get_YH  + 35, 'Factura No 000-003-01-'.$numero_venta);
		//$pdf->Text(55, $get_YH + 35, 'Caja No.: 1');
		$pdf->Text(4, $get_YH + 40, 'Fecha : '.$fecha_venta);
		$pdf->Text(4, $get_YH + 45, 'No. Ticket : '.$numero_comprobante);
		$pdf->Text(38, $get_YH  + 45, 'Cajero : '.substr($empleado, 0,5));
		//$pdf->Text(4, $get_YH + 50, 'Cliente : '.$nombre);
		//$pdf->Text(4, $get_YH + 55, 'RTN : '.$rtn);
		$pdf->SetFont('Arial', '', 9.2);
		$pdf->Text(2, $get_YH + 59, '------------------------------------------------------------------');

		$pdf->SetXY(2,$get_YH + 60);
		...
		$pdf->Text(57,$get_Y + 10,'L '.$subtotal);
		...
		$pdf->Text(57,$get_Y + 20,'L '.$exento);
		$pdf->Text(4,$get_Y + 25,'Importe gravado 15%:');
		$pdf->Text(57,$get_Y + 25,'L '.$subtotal);
		...
		$pdf->Text(57,$get_Y + 45,'L -'.$descuento);
		$pdf->Text(4,$get_Y + 50,'Total a pagar :');
		$pdf->SetFont('Arial','B',8.5);
		//$pdf->Text(54,$get_Y + 50,'L.');
		$pdf->Text(57,$get_Y + 50,'L '.$total);

		$pdf->Text(2, $get_Y+55, '-----------------------------------------------------------------------');
		$pdf->Text(4,$get_Y + 60,'Numero de Productos :');
		$pdf->Text(57,$get_Y + 60,$numero_productos);

		if($tipo_pago == 'EFECTIVO'){

		$pdf->Text(24,$get_Y + 65,'Efectivo :');
		$pdf->Text(57,$get_Y + 65,'L '.$efectivo);
		$pdf->Text(24,$get_Y + 69,'Cambio :');
		$pdf->Text(57,$get_Y + 69,'L '.$cambio);


		$pdf->Text(2, $get_Y+72, '-----------------------------------------------------------------------');
		$pdf->SetFont('Arial','BI',8.5);
		$pdf->Text(3, $get_Y+77, 'Precios en : '.$moneda);
		if($estado == '2'):
			$pdf->Text(3, $get_Y+82, 'Esta venta ha sido al credito');
			$pdf->SetFont('Arial','B',8.5);
		endif;
		$pdf->SetFont('Arial','B',8.5);
		...
		$pdf->Text(28, $get_Y+94, '*'.$numero_venta.'*');
		...
		$pdf->Text(24,$get_Y + 66,'Efectivo :');
		$pdf->Text(57,$get_Y + 66,'L '.$efectivo);

		$pdf->Text(20,$get_Y + 71,'No. Tarjeta :');
		$pdf->Text(40,$get_Y + 71,$numero_tarjeta);
		$pdf->Text(23,$get_Y + 76,'Debitado :');
		$pdf->Text(57,$get_Y + 76,'L '.$pago_tarjeta);

		$pdf->Text(2, $get_Y+78, '-----------------------------------------------------------------------');
		$pdf->SetFont('Arial','BI',8.5);
		$pdf->Text(3, $get_Y+83, 'Precios en : '.$moneda);
		$pdf->SetFont('Arial','',8.5);
		$pdf->Text(3, $get_Y+88, 'Venta realizada con dos metodos de pago');
		$pdf->SetFont('Arial','B',8.5);
		if($estado == '2'):
			$pdf->Text(3, $get_Y+93, 'Esta venta ha sido al credito');
			$pdf->SetFont('Arial','B',8.5);
		endif;
		$pdf->Text(19, $get_Y+98, 'GRACIAS POR SU COMPRA');
		$pdf->SetFillColor(0,0,0);
		$pdf->Code39(9,$get_Y+105,$numero_venta,1,5);
		$pdf->Text(28, $get_Y+104, '*'.$numero_venta.'*');
