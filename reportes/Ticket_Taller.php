<?php
	...
	$pdf->AddPage();
        ...
        if($diagnostico>""){
             ...
             $pdf->Text(18, $get_YH + 7, utf8_decode('Reporte Técnico'));
        }
        
        if($diagnostico == ""){
             $pdf->SetFont('Arial', '', 16);
             $pdf->Text(18, $get_YH + 7, 'Orden de servicio');
        }
       
       
        
        $pdf->SetFont('Arial', '', 8);
		$pdf->Text(5, $get_YH + 10, '---------------------------------------------------------------------');
		$pdf->SetY($get_YH + 11);
		$pdf->SetX(5);
		$pdf->SetFont('Arial', '', 8);
		...
		$pdf->MultiCell(65, 4.2, 'TELEFONO : '.$numero_telefono, 0,'L',0 ,1);
		$get_YC = $pdf->GetY();
		$pdf->SetXY(5,$get_YC + 1);
		...
		$pdf->Text($get_XL - 1, $get_YL  + 20, 'DEPOSITO REVISION : ' . number_format($deposito_revision, 2, '.', ','));
		$pdf->Text($get_XL - 1, $get_YL  + 25, 'DEPOSITO REPARACION : ' . number_format($deposito_reparacion, 2, '.', ','));
		...
		reparados que no se han retirado. Toda reparacion tiene un mes de garantia.
		Cargo minimo de L. 345 por Diagnostico'), 0,'C',0 ,1);
		
		$pdf->Text(10, $get_YL+112, '_________________________________');
    	$pdf->Text(25, $get_YL+116, 'Firma Cliente');
