<?php
  ...
  spl_autoload_register(function($className){
    ...
    $pdf->Cell(20,10,utf8_decode($a_nombre));
    $pdf->setXY(10,50);
    ...
    $pdf->SetFillColor(209,230,240);
    $pdf->Cell(23,5,'Cant.',0,0,'L',1);
    $pdf->Cell(108,5,'Producto',0,0,'C',1);
    $pdf->Cell(23,5,'Precio (Sin ISV)',0,0,'C',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->setX(10);
        $pdf->Cell(8,5,$column["cantidad"],0,0,'L',1);
        $pdf->MultiCell(100,5,$column["nombre_producto"],0,0,'L',1);
        //$pdf->Cell(108,5,$column["nombre_producto"],1,0,'L',1);
        $pdf->setX(141);
        $punitario = $column["precio_unitario"] / 1.15;
        $pdf->Cell(23,-5,number_format($punitario, 2, '.', ','),0,0,'C',1);
        //$pdf->Cell(23,-5,number_format($column["precio_unitario"], 2, '.', ','),0,0,'C',1);
        $pdf->Cell(23,-5,$column["exento"],0,0,'C',1);
       // $pdf->Cell(23,5,$column["descuento"],1,0,'C',1);
       $subtot = $column["importe"] / 1.15;
       $pdf->Cell(23,-5,number_format($subtot, 2, '.', ','),0,0,'C',1);
        //$pdf->Cell(23,-5,number_format($column["importe"], 2, '.', ','),0,0,'C',1);
        $pdf->Ln(3);
        $get_Y = $pdf->GetY();
      }
      ...
      $pdf->SetFont('Arial','',8.5);
      $pdf->Text(15,$get_Y + 12,'1 - COTIZACION VALIDA POR 15 DIAS, SUJETA A CAMBIOS');
      $pdf->Text(19.5,$get_Y + 16,'SIN PREVIO AVISO.');
      $pdf->Text(15,$get_Y + 23,'2 - TIEMPO DE ENTREGA : '.$entrega.'.');
      $pdf->Text(15,$get_Y + 30,'3 - FORMA DE PAGO: '.$tipo_pago.'.');
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'SUMAS','TB',0,'R',1);
      $pdf->SetFont('Arial','',8.5);
      $pdf->SetFillColor(255,255,255);
      $pdf->Cell(23,5,number_format($sumas, 2, '.', ','),'TRB',0,'C',1);
      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'ISV','B',0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->SetFont('Arial','',8.5);
      $pdf->Cell(23,5,number_format($iva, 2, '.', ','),'RB',0,'C',1);
      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'SUBTOTAL','B',0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->SetFont('Arial','',8.5);     
      $pdf->Cell(23,5,number_format($subtotal, 2, '.', ','),'RB',0,'C',1);

      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'RETENCION','B',0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->SetFont('Arial','',8.5);
      $pdf->Cell(23,5,$retenido,'RB',0,'C',1);
      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'TOTAL EXENTO','B',0,'R',1);
      $pdf->SetFont('Arial','',8.5);
      $pdf->SetFillColor(255,255,255);
      $pdf->Cell(23,5,number_format($total_exento, 2, '.', ','),'RB',0,'C',1);
      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'TOTAL DESCUENTO','B',0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->SetFont('Arial','',8.5);
      $pdf->Cell(23,5,$total_descuento,'RB',0,'C',1);
      $pdf->Ln(5);
      $pdf->setX(154);
      $pdf->SetFillColor(209,230,240);
      $pdf->SetFont('Arial','B',8.5);
      $pdf->Cell(33,5,'TOTAL PAGAR','B',0,'R',1);
      $pdf->SetFillColor(255,255,255);
      $pdf->SetFont('Arial','',8.5);
      $pdf->Cell(23,5,number_format($total, 2, '.', ','),'BR',0,'C',1);
   ...
   */    
    ...
    $pdf->Cell(30,6,'Formas de pago:');
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(90,6,'Efectivo, Tarjeta, Transferencia, depositos, Cheques certificados a nombre de Ditec S de RL');
