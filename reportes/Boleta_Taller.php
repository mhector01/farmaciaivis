<?php
  ...
  spl_autoload_register(function($className){
      ...
      foreach ($datos as $row => $column) {
        ...
        $numero_telefono = substr($numero_telefono, 0, 4).'-'.substr($numero_telefono, 4, 5);
      }
    ...
    $pdf->Image('https://ditechonduras.com/wp-content/uploads/2020/10/cropped-Logo.png',175,1,40,0,'PNG');
    ...
    $pdf->setXY(10,6);
    $pdf->Cell(50,20,$direccion_empresa);
    $pdf->setX(10);
    $pdf->Cell(18,30,utf8_decode('Teléfono : '));
    $pdf->Cell(2,30,$numero_nrc);
    $pdf->setX(10);
    ...
    $pdf->Cell(20,10,utf8_decode($nombre_cliente));
    $pdf->SetFont('Arial','',13);
    $pdf->setXY(10,50);
    ...
    $pdf->Cell(2,10,utf8_decode('Reporte Técnico'));
    }
    
     if($diagnostico == ""){
        $pdf->setXY(80,62);
    $pdf->SetFont('Arial','',20);
    $pdf->Cell(2,10,utf8_decode('Orden de ingreso'));
    }
    ...
    $pdf->Ln(10);
    $pdf->SetFont('Arial','',8);
    $pdf->SetFillColor(208,228,247);
    $pdf->Cell(50,5,'Equipo',1,0,'L',1);
    $pdf->Cell(25,5,'Modelo',1,0,'C',1);
    $pdf->Cell(25,5,'Serie',1,0,'C',1);
    $pdf->Cell(70,5,'Falla reportada',1,0,'C',1);
    ...
    $pdf->Ln(15);
      ...
      $pdf->Text(15,$get_Y2 + 24,'1 - Precio final se dara junto al diagnostico ');
      $pdf->Text(15,$get_Y2 + 28,utf8_decode('2 - Tiene dos meses para retirar el equipo luego de confirmar el diagnostico'));
      $pdf->Text(15,$get_Y2 + 32,utf8_decode('3 - Toda reparacion tiene un mes de garantia'));
      $pdf->setX(154);
      $pdf->SetFillColor(208,228,247);
      $pdf->SetFont('Arial','B',8.5);
    ...
    $pdf->SetFont('Arial','',10);
    $pdf->setXY(10,200);
    $pdf->Cell(22,6,'Firma cliente: ');
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(30,6,'______________________');



    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,230);
    $pdf->Cell(30,6,'- Despues de 90 dia, si el equipo no es retirado tras confirmar su revision o reparacion, no nos hacemos responsables por la devolucion');
    
    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,235);
    $pdf->Cell(30,6,'- Se cobrara recargo de L. 100 al mes por equipo no retirado');

    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,240);
    $pdf->Cell(30,6,'- No nos hacemos responsables por respaldo de informacion, es responsabilidad del cliente guardar su informacion antes de ingresar el equipo.');

    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,245);
    $pdf->Cell(30,6,'- Cargo minimo de L. 345 por DIAGNOSTICO equipo en general, fotocopiadoras e impresoras laser L. 690.');

    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,250);
    $pdf->Cell(30,6,'- Despues de haber sido reparado el equipo, debera cancelar un minimo de L.345, este costo se aplicara dependiendo el tiempo invertido en el equipo.');

    $pdf->SetFont('Arial','',8.5);
    $pdf->setXY(10,253);
    $pdf->Cell(30,6,'   SIN INCLUIR REPUESTOS.');

    
    $pdf->Image('https://ditechonduras.com/wp-content/uploads/2022/06/sello.png',60,150,45,0,'PNG');
    if($tecnico == 'HECTOR MENDOZA'){
    $pdf->Image('https://ditechonduras.com/wp-content/uploads/2022/06/FirmaDigital.png',80,155,45,0,'PNG');

    }

    if($tecnico == 'JHONY FLORES'){
    $pdf->Image('https://ditechonduras.com/wp-content/uploads/2022/07/firma-Jhony.png',80,155,45,0,'PNG');
    }
      ...
      $pdf->Output('I','Orden_'.$numero_orden.'.pdf');
