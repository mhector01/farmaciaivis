<?php
	...
	$pdf->AddPage();
		...
		$pdf->Text(4, $get_YH  + 15, 'Cajero : '.substr($empleado, 0,15));
		$pdf->SetFont('Arial', '', 9.2);
		...
		$pdf->SetFont('Arial','B',8.5);
		$pdf->Cell(30,4,'Descripcion',0,0,'L',1);
		$pdf->Cell(14,4,'Cantidad',0,0,'L',1);
		...
		while($row = $detalle->fetch(PDO::FETCH_ASSOC)) {
		 $item = $item + 1;
			...
			$pdf->MultiCell(34,4,$row['descripcion'],0,'L');
			...
			$pdf->Cell(12,-5,$row['precio_unitario'],0,0,'L',1);
			$pdf->Cell(12,-5,$row['importe'],0,0,'L',1);
			$pdf->Ln(4.5);
			$get_Y = $pdf->GetY();
		}
